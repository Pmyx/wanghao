<template>
    <div>
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>酷站分类</el-breadcrumb-item>
        </el-breadcrumb>
        ...
        <el-table ref="multipleTable" :data="site_nodes" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        methods: {
            ...
            handleDelete(index, row) {
                if (confirm('are you sure?')) {
                    ...
                    axios.delete(`/admin/site_nodes/${id}`).then((res) => {
                        this.$notify({
                            title: '成功',
                            message: '删除成功',
                            type: 'success'
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        if (this.is_type.value == 1) {
                            ...
                            axios.post(`/admin/site_nodes`, this.ruleForm).then((res) => {
                                //提示消息
                                this.$notify({
                                    title: '成功',
                                    message: '新增成功',
                            ...
                            //console.log(this.ruleForm);return ;
                            axios.put(`/admin/site_nodes/${this.ruleForm.id}`, this.ruleForm).then((res) => {
                                //提示消息
                                this.$notify({
                                    title: '成功',
                                    message: '编辑成功',
            ...
            change_sort(index, row) {
                ...
                axios.patch(`/admin/site_nodes/change_sort/${row.id}`, {sort: row.sort}).then((res) => {
                    //提示消息
                    this.$notify({
                        title: '成功',
                        message: '排序编辑成功',
            ...
            del_all() {
                ...
                if (confirm('确认删除所选吗？')) {
                    ...
                    axios.post(`/admin/site_nodes/del_all`,{checked_id:checked_id}).then((res) => {
                        this.$notify({
                            title: '成功',
                            message: '删除成功',
                            type: 'success'
