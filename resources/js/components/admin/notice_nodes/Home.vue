<template>
    <div>
        ...
        <el-table ref="multipleTable" :data="notice_nodes" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        methods: {
            ...
            handleDelete(index, row) {
                if (confirm('are you sure?')) {
                    ...
                    axios.delete(`/admin/notice_nodes/${id}`).then((res) => {
                        this.$notify({
                            title: '成功',
                            message: '删除成功',
                            type: 'success'
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        if (this.is_type.value == 1) {
                            ...
                            axios.post(`/admin/notice_nodes`, this.ruleForm).then((res) => {
                                //提示消息
                                this.$notify({
                                    title: '成功',
                                    message: '新增成功',
                            ...
                            //console.log(this.ruleForm);return ;
                            axios.put(`/admin/notice_nodes/${this.ruleForm.id}`, this.ruleForm).then((res) => {
                                //提示消息
                                this.$notify({
                                    title: '成功',
                                    message: '编辑成功',
            ...
            change_sort(index, row) {
                ...
                axios.patch(`/admin/notice_nodes/change_sort/${row.id}`, {sort: row.sort}).then((res) => {
                    //提示消息
                    this.$notify({
                        title: '成功',
                        message: '排序编辑成功',
            ...
            del_all() {
                ...
                if (confirm('确认删除所选吗？')) {
                    ...
                    axios.post(`/admin/notice_nodes/del_all`,{checked_id:checked_id}).then((res) => {
                        this.$notify({
                            title: '成功',
                            message: '删除成功',
                            type: 'success'
